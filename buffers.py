...
class RolloutBuffer(BaseBuffer):
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        ...
        for step in reversed(range(self.buffer_size)): 
            ...
            # if step == 0:
            #     alpha = 1.0
            # last_gae_lam = (delta + self.gamma * next_non_terminal * last_gae_lam * step) * alpha
            
            # Weighted moving average
            # alpha = 1/ (total_step + step + 1)
            # if step == 0:
            #     alpha = 1.0
            # total_step = (1 + step) * step / 2
            # last_gae_lam = (delta * (step + 1) + self.gamma * next_non_terminal * \
                            # last_gae_lam * total_step)/ (total_step + step + 1)
            
            # Exponential moving average (Holt linear)
            # alpha = 0.8
            # if step == 0:
            #     alpha = 1.0
